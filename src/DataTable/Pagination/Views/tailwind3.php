            ...
            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                <div>
                    <p class="text-sm text-gray-700 leading-5 dark:text-gray-400">
                        <span>
                            {!! __('erickcomp_lw_data_table::messages.pagination.showing') !!}
                            {!! __('erickcomp_lw_data_table::messages.pagination.showing_from') !!}
                        </span>
                        <span class="font-medium">{{ $paginator->firstItem() }}</span>
                        <span>{!! __('erickcomp_lw_data_table::messages.pagination.showing_to') !!}</span>
                        <span class="font-medium">{{ $paginator->lastItem() }}</span>
                        <span>{!! __('erickcomp_lw_data_table::messages.pagination.showing_of') !!}</span>
                        <span class="font-medium">{{ $paginator->total() }}</span>
                        <span>{!! __('erickcomp_lw_data_table::messages.pagination.showing_results') !!}</span>
                    </p>
                </div>
                        ...
                        <span>
                            ...
                            @if ($paginator->onFirstPage())
                                <span aria-disabled="true"
                                    aria-label="{{ __('erickcomp_lw_data_table::pagination.previous') }}">
                                    <span
                                    ...
                                    <span wire:key="paginator-{{ $paginator->getPageName() }}-page{{ $page }}">
                                        ...
                                        @else
                                            <button type="button" wire:click="gotoPage({{ $page }}, '{{ $paginator->getPageName() }}')"
                                                ...
                                                aria-label="{{ __('erickcomp_lw_data_table::pagination.go_to_page', ['page' => $page]) }}">
                                                {{ $page }}
                                            </button>
                                        @endif
                                    </span>
                        ...
                        <span>
                            ...
                            @if ($paginator->hasMorePages())
                                <button type="button" wire:click="nextPage('{{ $paginator->getPageName() }}')"
                                    ...
                                    aria-label="{{ __('erickcomp_lw_data_table::pagination.next') }}">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd"
                            ...
                            @else
                                <span aria-disabled="true" aria-label="{{ __('erickcomp_lw_data_table::pagination.next') }}">
                                    <span

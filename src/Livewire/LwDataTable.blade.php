...
<div class="lw-dt-container">
    <table {{ $dataTable->tableAttributes }}>
        <thead {{ $dataTable->theadAttributes }}>
            <tr {{ $dataTable->theadTrAttributes }}>
                @foreach ($dataTable->columns as $column)
                    <th {{ $thAttributes($column->thAttributes, $dataTable->thAttributes) }}
                        {!! $onClickSortableColumn($column) !!}
                    >
                        {{ $column->title }}
                        @if ($column->isSortable())
                            @php
                                $columnSortClass = $column->name === $sortBy
                                    ? Str::kebab('lw-dt-sort-' . \strtolower(empty($sortDir) ? 'none' :$sortDir))
                                    : 'lw-dt-sort-none';
                            @endphp
                            <span class="lw-dt-sort {{ $columnSortClass }}"></span>
                        @endif
                    </th>
            ...
            @if($dataTable->hasSearchableColumns())
                <tr {{ $dataTable->theadSearchTrAttributes }}>
                    @foreach ($dataTable->columns as $column)
                        <th {{ $dataTable->theadSearchTdAttributes }}>
        ...
        <tbody {{$dataTable->tbodyAttributes }}>
            @foreach ($rows as $row)
                <tr {{ $dataTable->tbodyTrAttributes }}>
                    @foreach ($dataTable->columns as $column)
                        <td {{ $column->tdAttributes ?? '' }}>
                            @if($column instanceof DataColumn)
                                {{ $row->{$column->dataField} }}
                            @elseif($column instanceof CustomRenderedColumn)
                                {!! Blade::render($column->customRendererCode, ['__dataTableRow' => $row]) !!}
...
@if($dataTable->withoutDefaultStyles === false)
    @assets
        <style>
            .lw-dt-container table {
                border: 1px solid #ccc;
                padding: 8px;
            }

            .lw-dt-container table th, table td {
                border: 1px solid #ccc;
                padding: 8px;
            }

            .lw-dt-container table th {
                background-color: #f4f4f4;
            }

            .lw-dt-container th:has(.lw-dt-sort) {
                cursor: pointer;
            }

            .lw-dt-container .lw-dt-sort {
            ...
            .lw-dt-container .lw-dt-sort.lw-dt-sort-none {
            ...
            .lw-dt-container .lw-dt-sort.lw-dt-sort-asc {
            ...
            .lw-dt-container .lw-dt-sort.lw-dt-sort-desc {
    ...
    @endassets
@elseif($dataTable->withSortingIndicators)
    @assets
        <style>
            .lw-dt-container th:has(.lw-dt-sort) {
                cursor: pointer;
